...
async function heroSpacing(){if(await page.locator('.hero h1').count()){assert.ok(await page.evaluate(()=>{if(innerWidth>700)return true;const bottom=document.querySelector('.hero h1').getBoundingClientRect().bottom,cta=document.querySelector('.hero-actions').getBoundingClientRect().top;return [...document.querySelectorAll('.geometry-hero>*:not(.draft-grid)')].every(e=>{const r=e.getBoundingClientRect();return r.top>bottom+5&&r.bottom<cta-5;});}), 'Hero artwork must clear the headline and start button');}}
